...
type program = Main of string * string * c
...
(* Commands AST Node eval function *)
...
(* AST Root eval *)
let eval_prg (p: program) (input: int) : env =
  match p with
  | Main (x, y, c) -> 
...
let print_var (e: env) (var: var) : unit =
  match e var with
  | Some value -> Printf.printf "Variable %s: %d\n" var value
  | None -> Printf.printf "Variable %s not found in the environment\n" var
;;
